...
// WorkflowStatus.RegisteringVoters in VotingPlus.sol — the stage a freshly
// created election starts in.
const REGISTERING_VOTERS = 0;

/** A fresh factory plus a few named signers. */
async function deployFactoryFixture() {
  const [admin, alice, bob, carol] = await ethers.getSigners();
  // Fully typed via the ethers bindings Hardhat generates under types/.
  const factory = await ethers.deployContract("VotingFactory");
  return { factory, admin, alice, bob, carol };
}
...
describe("VotingFactory", function () {
  ...
  it("makes the createVoting caller the admin of the new election", async function () {
    ...
    expect(await election.currentWorkflowStatus()).to.equal(REGISTERING_VOTERS);
  });
